...
winedata %>%
 ...
 ylab("Average Price") +
 xlab("rating") +
 ggtitle("Affect of Average Rating on price") 
```
...
We now repeat the operation we did above but with our filtered dataset with outlier prices removed. Notice how France and Italy have the steepest lines indicating that increases in ratings have more of an effect on the average price then in the US, Spain or Chile. 
...
Now we are going to investigate the linear regression properties of avg_rating versus average_price grouped by countries. We want to determine whether there is a relationship between rating and price using p values. the lm function here. The way the function works
```{r}

auto_fit <- lm(avg_price~avg_rating, data=winedata)
auto_fit
```
